...
use crate::tools::handlers::multi_agents_v2::MultiAgentV2ToolOptions;
use crate::tools::handlers::multi_agents_v2::register_multi_agent_v2_tools;
...
fn add_collaboration_tools(context: &CoreToolPlanContext<'_>, registry: &mut ToolRegistry) {
    ...
    if collab_tools_enabled(turn_context, context.model_info) {
        if multi_agent_v2_enabled(turn_context) {
            ...
            let provider_is_openai = turn_context.provider.info().is_openai();
            register_multi_agent_v2_tools(
                registry,
                MultiAgentV2ToolOptions {
                    namespace: tool_namespace,
                    exposure,
                    provider_is_openai,
                    external_bridge_enabled: namespace_tools_enabled(turn_context)
                        && turn_context.config.has_authorized_external_subagent_role(),
                    spawn: SpawnAgentToolOptions {
                    ...
                    wait_agent: turn_context
                        .config
                        .multi_agent_v2
                        .wait_agent_enabled
                        .then_some(context.wait_agent_timeouts),
                },
            );
        } else {
            let agent_type_description =
